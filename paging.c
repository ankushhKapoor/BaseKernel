...
void paging_init() {
	...
	for(int currPDE=0 ; currPDE<PDE_NUM ; currPDE++) {
		...
		for(int currPTE=0 ; currPTE<PTE_NUM; currPTE++, curr_page_frame++)
				pagetable[currPTE] = create_page_entry(curr_page_frame*4096, 1, 0, 0, 1, 1, 0, 0, 0);	// page frame = used, cache = enabled, policy = write through, belongs to supervisor privilege level, page size = 4KB
