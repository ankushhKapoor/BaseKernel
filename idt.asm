...
isr_basic:
	...
	pop eax	; or add esp, 4 (this is better as popped element is not stored)
...
irq_basic:
	...
	mov al, 0x20
	out 0x20, al	; End of Interrupt (EOI) cmd

	cmp byte [esp], 40d
	jnge irq_basic_end	; jump if no slave PIC interrupt

	mov al, 0xa0
	out 0x20, al	; send EOI cmd to slave PIC
...
idt:
	dw isr_0, 8, 0x8e00, 0x0000	; "isr_segment_selector": 8, "present": true, "dpl": 0, "gate_descriptor_size": "32-bit", "interrupt_gate": true
...
idtr:
	idt_size_in_byte	:	dw idtr - idt	; not hardcoded like gdt because this handles size automatically if new interrupt is added (only works if idtr is just next to idt)
